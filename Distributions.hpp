 ...
 * class asams::DistributionBase.<br> Distribution function root names are preceeded
...
static T IncompleteReqularizedBeta(const T &x, const T &a, const T &b) {
  ...
  return asams::IncompleteBeta<T > (x, a, b) / asams::RegularizedBeta<T > (T(1), a, b);
}
...
const T pnorm(const T &x, const T &mean, const T &sd) {
  return T(0.5)*(T(1.0) + asams::Erf<T > ((x - mean) / std::sqrt(T(2.0) * sd * sd)));
}
...
const T qnorm(const T &probability, const T &mean, const T &sd) {
  //          return (T(22619537.0) * fabs(sd) * asams::InverseErf(T(T(2.0) * probability-T(1.0))) + T(15994428.0) * mean) / T(15994428.0);
  double guess = mean;
  double high = (100);
  double low = (-100);
  double x = guess;
...
const T dbeta(const T &x, const T &a, const T &b) {
  if (x <= T(0.0) || x >= T(1.0)) {
    ...
    T ret = T(-1) * asams::BetaLn<T > (a, b) + (a - T(1.0)) * std::log(x)+(b - T(1.0)) * std::log(T(1.0) - x);
...
const T pbeta(const T &x, const T &a, const T &b) {
  if (x <= T(0)) {
    ...
    T ret = asams::IncompleteBeta<T > (x, a, b);
    return ret;
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dbeta(x, this->GetAlpha(), this->GetBeta());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pbeta(x, this->GetAlpha(), this->GetBeta());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qbeta(probability, this->GetAlpha(), this->GetBeta());
  }
   ...
   */
  const T Random() const {
    return asams::rbeta(this->GetAlpha(), this->GetBeta());
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dbinom<T > (x, this->GetNumberOfTrials(), this->GetProbabilityOfSuccess());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pbinom<T > (x, this->GetNumberOfTrials(), this->GetProbabilityOfSuccess());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qbinom<T > (probability, this->GetNumberOfTrials(), this->GetProbabilityOfSuccess());
  }
   ...
   */
  const T Random() const {
    return asams::rbinom<T > (this->GetNumberOfTrials(), this->GetProbabilityOfSuccess());
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dcauchy<T > (x, this->GetLocation(), this->GetScale());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pcauchy<T > (x, this->GetLocation(), this->GetScale());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qcauchy<T > (probability, this->GetLocation(), this->GetScale());
  }
   ...
   */
  const T Random() const {
    return asams::rcauchy<T > (this->GetLocation(), this->GetScale());
  }
...
const T dpois(const T &x, const T &lambda) {
  return (exp (x * std::log(lambda) - lambda - asams::GammaLn(x + T(1))));
}
...
const T ppois(const T &x, const T &lambda) {
  ...
  for (T i = T(0.0); i < std::floor(x) + T(1); i++) {
    sum += (std::pow(lambda, i) / T(asams::Factorial(i)));
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dpois<T > (x, this->GetLambda());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::ppois<T > (x, this->GetLambda());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qpois<T > (probability, this->GetLambda());
  }
   ...
   */
  const T Random() const {
    return asams::rpois<T > (this->GetLambda());
  }
...
const T pgamma(const T &x, const T &shape, const T &scale) {
  ...
  if (shape > T(0) && scale > T(0)) {
    // return   (T(1.0)/asams::Gamma(shape)) *asams::IncompleteGamma(shape,x/scale);
    
    T sum = T(0);
    
    for (int i = 0; i < shape; i++) {
      sum += T(1.0) / asams::Factorial(i) * std::pow((x / scale), (T(i))) * std::exp(T(-1)*(x / scale));
...
const T dgamma(const T &x, const T &shape, const T &scale) {
  return (T(1.0) / std::pow(scale, shape))*(T(1.0) / asams::Gamma<T > (shape)) * std::pow(x, shape - T(1)) * std::exp(T(-1) * x / scale);
}
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dgamma<T > (x, this->GetShape(), this->GetScale());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pgamma<T > (x, this->GetShape(), this->GetScale());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qgamma<T > (probability, this->GetShape(), this->GetScale());
  }
   ...
   */
  const T Random() const {
    return asams::rgamma<T > (this->GetShape(), this->GetScale());
  }
...
const T pchisq(const T &x, const T &df) {
  ...
  return (T(1.0) / (asams::Gamma(df / T(2))))*asams::LowerIncompleteGamma(df / T(2.0), x / T(2.0));
  //return T(1.0) - (T(1.0) / asams::Gamma(df / T(2.0))) * asams::LowerIncompleteGamma(df / T(2.0), x / T(2.0)); alternative version
}
...
const T dchisq(const T &x, const T &df) {
  if (x >= T(0)) {
    //            return (T(1.0) / (std::pow(T(2.0), df / T(2.0)) * asams::Gamma<T > (df / T(2.0))))*
    //                    std::pow(x, (df / T(2.0)) - T(1)) * std::exp(T(-1) * x / T(2.0)); //alternative version
    return ((T(1) / std::pow(T(2), (df / T(2.0))) * asams::Gamma(df / T(2.0))) * std::pow(x, ((df / T(2.0)) - T(1.0))) * std::exp(T(-1.0) * x / T(2)));
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dchisq(x, this->GetDegreesOfFreedom());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pchisq(x, this->GetDegreesOfFreedom());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qchisq(probability, this->GetDegreesOfFreedom());
  }
   ...
   */
  const T Random() const {
    return asams::rchisq(this->GetDegreesOfFreedom());
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dexp(x, this->GetRate());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pexp(x, this->GetRate());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qexp(probability, this->GetRate());
  }
   ...
   */
  const T Random() const {
    return asams::rexp(this->GetRate());
  }
...
const T pf(const T &x, const T &d1, const T &d2) {
  return asams::IncompleteReqularizedBeta((d1 * x) / (d1 * x + d2), d1 / T(2), d2 / T(2));
}
...
const T df(const T &x, const T &d1, const T &d2) {
  if (x >= T(0) && d1 > T(0) && d2 > T(0)) {
    return std::sqrt((std::pow(x*d1, d1)*(d2 * d2)) / std::pow(x * d1 + d2, d1 + d2)) / (x * asams::Beta<T > (d1 / T(2.0), d2 / T(2.0)));
  }
  return T(0);
}
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::df(x, this->GetDegreesOfFreedom1(), this->GetDegreesOfFreedom2());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pf(x, this->GetDegreesOfFreedom1(), this->GetDegreesOfFreedom2());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qf(probability, this->GetDegreesOfFreedom1(), this->GetDegreesOfFreedom2());
  }
   ...
   */
  const T Random() const {
    return asams::rf(this->GetDegreesOfFreedom1(), this->GetDegreesOfFreedom2());
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dgeom(x, this->GetProbability());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::pgeom(x, this->GetProbability());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qgeom(probability, this->GetProbability());
  }
   ...
   */
  const T Random() const {
    return asams::rgeom(this->GetProbability());
  }
...
const T dhyper(const T &x, const T &m, const T &n, const T &k) {
  return (T(asams::Choose(m, x)) * T(asams::Choose(n, (k - x)))) / T(asams::Choose((m + n), k));
}
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dhyper(x, this->GetM(), this->GetN(), this->GetK());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::phyper(x, this->GetM(), this->GetN(), this->GetK());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qhyper(probability, this->GetM(), this->GetN(), this->GetK());
  }
   ...
   */
  const T Random() const {
    return asams::rhyper(this->GetM(), this->GetN(), this->GetK());
  }
...
public:
   ...
   */
  const T Probability(const T &x) const {
    return asams::dlogis(x, this->GetLocation(), this->GetScale());
  }
   ...
   */
  const T Cumulative(const T &x) const {
    return asams::plogis(x, this->GetLocation(), this->GetScale());
  }
   ...
   */
  const T Inverse(const T &probability) const {
    return asams::qlogis(probability, this->GetLocation(), this->GetScale());
  }
   ...
   */
  const T Random() const {
    return asams::rlogis(this->GetLocation(), this->GetScale());
  }
...
const T plnorm(const T &x, const T &meanLog, const T &sdLog) {
  if (x >= T(0)) {
    return T(.5) + T(.5) * asams::Erf<T > ((std::log(x) - meanLog) /
...
const T dnbinom(const T &x, const size_t &n, const T &p) {
  ...
  size_t top = x + n - 1;
  size_t bottom = x;
  size_t combinations = asams::Factorial(top) / (asams::Factorial(bottom) * asams::Factorial(top - bottom));
...
const T pnbinom(const T &x, const size_t &n, const T &p) {
  ...
  return sum;
  //        return asams::IncompleteBeta<T > (p, x, n);
...
const T pt(const T &x, const T &df) {
  T xt = df / (x * x + df);
  return T(1) - T(.5) * asams::IncompleteBeta<T > (xt, df / T(2), T(.5));
...
const T dt(const T &x, const T &df) {
  return (asams::Gamma<T > ((df + T(1)) / T(2)) /
          (std::sqrt(df * (T(M_PI))) * asams::Gamma<T > (df / T(2))))*
...
template<class T>
class WeibullDistribution : public DistributionBase<T> {
};
